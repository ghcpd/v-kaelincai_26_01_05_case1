...
These tests are designed to FAIL and expose the equipment validation bug.
"""
...
class TestEquipmentValidationBug:
    ...
    These tests will FAIL because the system incorrectly validates equipment requirements.
    """
    ...
    def test_bug_partial_equipment_match_should_fail(self, booking_system):
        """
        🐛 TEST CASE 1: Expose the equipment validation bug
        
        Bug: System allows booking when room has SOME but not ALL required equipment.
        
        Scenario:
        - Room A has: {PROJECTOR}
        - Request needs: {PROJECTOR, VIDEO_CONF}
        - Expected: Booking should FAIL (missing VIDEO_CONF)
        - Actual: Booking SUCCEEDS (bug - only checks if ANY equipment matches)
        """
        ...
        # Expected: booking should fail because VIDEO_CONF is missing
        # Actual: booking succeeds because room has PROJECTOR (partial match)
    ...
    def test_bug_client_meeting_scenario(self, booking_system):
        ...
        # This will FAIL - booking succeeds when it shouldn't
        assert result.success is False, \
    ...
    def test_bug_multiple_missing_equipment(self, booking_system):
        ...
        Bug exposure: Room has PROJECTOR, request needs VIDEO_CONF + WHITEBOARD + AUDIO
        Expected: Should fail (missing 3 items)
        Actual: Should fail (room has no overlap) - this one might actually work correctly
        """
        request = BookingRequest(
        ...
        # This should correctly fail (no overlap between required and available)
        assert result.success is False, \
