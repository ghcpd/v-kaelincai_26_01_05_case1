...
This module contains the main booking system with a DELIBERATELY PLANTED BUG
for testing purposes. The bug is in the equipment validation logic.
"""
...
class MeetingRoomBookingSystem:
    ...
    def _validate_equipment_requirements(self, room: MeetingRoom, 
        ...
        🐛 BUG PLANTED HERE: This method has incorrect logic that allows bookings
        even when equipment requirements are not met!
        ...
        # It checks if there's ANY overlap between required and available equipment,
        # but it should check if ALL required equipment is available.
        # 
        # Current (BUGGY) logic: Returns True if room has AT LEAST ONE required item
        # Correct logic should be: Returns True ONLY if room has ALL required items
    ...
    def book_room(self, request: BookingRequest, preferred_room_id: Optional[str] = None) -> BookingResult:
        ...
        if preferred_room_id:
            ...
            # 🐛 BUG WILL BE TRIGGERED HERE when equipment validation is called
