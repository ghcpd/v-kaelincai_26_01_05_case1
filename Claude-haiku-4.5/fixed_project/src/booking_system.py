"""
Meeting Room Booking System - Core booking logic (FIXED VERSION).

This module contains the booking system with the equipment validation bug fixed.
The bug fix changes the equipment validation from checking ANY overlap to checking
that ALL required equipment is present.
"""
...
class MeetingRoomBookingSystem:
    ...
    def _validate_equipment_requirements(self, room: MeetingRoom, 
        ...
        FIXED VERSION: This method now correctly checks that the room has
        ALL required equipment, not just ANY of the required equipment.
        ...
        # The fix: Change from "has_any_equipment = bool(required_equipment & room.equipment)"
        # to: "all_equipment_available = required_equipment.issubset(room.equipment)"
        # This ensures ALL required items are present, not just ANY overlap
    ...
    def book_room(self, request: BookingRequest, preferred_room_id: Optional[str] = None) -> BookingResult:
        ...
        if preferred_room_id:
            ...
            # Equipment validation (with fixed logic)
